...
type Article struct {
	...
	CreatedAt time.Time `orm:"column(createdAt);type(timestamp with time zone);null"`
	UpdatedAt time.Time `orm:"column(updatedAt);type(timestamp with time zone);null"`
...
func GetAllArticle(query map[string]string, fields []string, sortby []string, order []string,
	...
	o := orm.NewOrm()
	qs := o.QueryTable(new(Article)).RelatedSel()
	// query k=v
	for k, v := range query {
